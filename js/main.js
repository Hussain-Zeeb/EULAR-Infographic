...
// GSAP animation for circle-arc-wrapper
  ...
  // Animate full subheading opacity with GSAP
  if (window.gsap && window.ScrollTrigger) {
    document.querySelectorAll('.subheading, h2').forEach((el, i) => {
      gsap.fromTo(el,
        { opacity: 0 },
        {
          opacity: 1,
  ...
  // Animate .subheading: fade in and move from left to right
  if (window.gsap && window.ScrollTrigger) {
    ...
    // Animate h2: fade in and move from left to right
  ...
  // Refresh ScrollTrigger on resize and periodically
if (window.ScrollTrigger) {
  ...
  window.addEventListener('resize', () => {
    ...
    }, 3000); // 3 second delay after resize
  });
}

window.addEventListener('load', function() {
  // Scroll to the anchor again to ensure correct position
  ...
  if (window.ScrollTrigger) {
    setTimeout(() => {
      ScrollTrigger.refresh();
    }, 2000); // slight delay to allow layout to settle
  }
});
